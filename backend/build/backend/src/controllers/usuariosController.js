...
class UsuariosController {
    ...
    readOne(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const usuario = yield database_1.default.query('select * from usuario where login = ?', [req.params.usuario]);
            res.json(usuario);
        });
    }
    readLogin(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (usuarioLogin.length == 0) {
                ...
                if (bcrypt.compareSync(usuario.password, usuarioLogin[0].password)) {
                    ...
                    const accessToken = jwt.sign({ usuario: usuarioLogin[0].login }, SECRET_KEY, { expiresIn: expiresIn });
